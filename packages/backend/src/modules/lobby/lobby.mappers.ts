...
export function toLobbyMemberPublic(member: MemberWithUser, hostId: string): LobbyMemberPublic {
  return {
...
export function toLobbyDetails(lobby: LobbyWithMembersAndHost, viewerUserId: string): LobbyDetails {
  ...
  return {
    ...summary,
    members: lobby.members.map((m) => toLobbyMemberPublic(m, lobby.hostId)),
  };
}
